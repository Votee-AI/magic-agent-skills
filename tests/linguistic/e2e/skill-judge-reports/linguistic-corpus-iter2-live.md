# Skill Evaluation Report: magic-linguistic-corpus (iteration 2 — LIVE)
...
> **Replaces**: `magic-linguistic-corpus-iter1.md` (simulated 102/A−)
...
| **D2 Mindset + Procedures** | **13** | 13 | 0 | 15 | 8-step workflow (L45-100) with explicit MANDATORY READ pointers at Steps 1/3/5/6 (L47, L56, L66, L76); each step is procedural not descriptive. Cross-skill routing explicit (magic-linguistic-ethics at Step 2, magic-linguistic-scripts at Step 4, magic-linguistic-tokenize at Step 8 hand-off). Step 5 dedup config has concrete defaults (num_perm=256, threshold=0.9 LR / 0.85 general, shingle 5 Latin / 3 Han-Indic). Step 8 produces concrete manifest template. **Held at 13**: Step 7 (Register-balance audit) has thresholds (Bible >30%, news >70%) but no script + no worked example; Step 4 (Unicode normalization) is a one-line punt to magic-linguistic-scripts without scope-delineation; missing explicit "if X happens, fall back to Y" recovery procedures. |
...
This skill, unlike magic-linguistic-ethics (where iter-1 over-credited by 4 pts), is **honestly priced** at the simulated level. The simulated rubric was conservative in its own favor on D4 and the live read corrects it.
...
**Baseline (no skill)**: would likely say "Cebuano Wikipedia has known quality issues; size alone isn't a great signal — also check actual benchmark scores." Likely passes assertions 1, 3, 4. May pass assertion 2 weakly with generic "automated content" but would NOT name **Lsjbot** specifically and would NOT generalize to **Waray + Volapük**. Would NOT cite the dedup amplification (5-10× smaller after dedup at 0.9). Would NOT route to magic-linguistic-scope for proper class assessment.

**Delta: substantive.** Skill names the actual bot (Lsjbot), names the related languages (Waray, Volapük), provides the dedup-amplification quantitative claim, and routes to the correct sibling skill (magic-linguistic-scope) for class assessment. The named-bot specificity is the kind of expert detail baseline systems miss.
...
3. **Cross-skill routing** (magic-linguistic-scope for class assessment; ethics for license)
