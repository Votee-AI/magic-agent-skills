...
**Mandatory steps:**
1. Pull catalogs from `linguistic-corpus` (OLDI, CulturaX, MADLAD-400, Glot500, regional community sources).
2. For MT: design parallel-data strategy via `linguistic-bitext`. Class 0-2 may need Bible-text bootstrapping; class 3-5 may use NLLB-style mining.
3. Run `linguistic-scripts` normalization audit on raw data BEFORE tokenizer training.
4. Run `linguistic-tokenize` fertility audit on existing/baseline tokenizer; recommend vocab extension if fertility > 3.0 vs English.
5. Per-dataset ethics check via `linguistic-ethics` (license, attribution, sacred-text, FPIC).
...
1. Morphology if MRL → `linguistic-morph`.
2. Syntactic eval if testing structural generalization → `linguistic-syntax`.
3. SRL/MWE coverage if RAG/structured extraction → `linguistic-semantics`.
4. Discourse if long-context generation/summarization → `linguistic-discourse`.
5. Speech if oral data sources or G2P needed → `linguistic-speech`.
6. Annotation workflow if creating gold data → `linguistic-annotate`.
...
**Mandatory steps:**
1. Choose benchmarks via `linguistic-eval` (avoid BLEU as primary on MRLs).
...
**Mandatory steps:**
1. License-compliance audit via `linguistic-ethics`.
