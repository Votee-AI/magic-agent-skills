...
Loaded by `linguistic-semantics` Step 2.
...
For low-resource: expect to either (a) use cross-lingual transfer from English SRL with frame alignment via Predicate Matrix, or (b) skip SRL and rely on dependency parsing (`linguistic-syntax`) + sense lookup.
