...
**When NOT to use:** for monolingual corpus → `linguistic-corpus`. For tokenizer fertility on the bitext output → `linguistic-tokenize`. For ethics/license per source → `linguistic-ethics`.
...
Route to `linguistic-scope` if not already done. Pair-level resource class (per-direction) determines the recipe.
...
Hand off to `linguistic-tokenize` for fertility audit on the bitext target side, and `linguistic-transfer` for adapter/LoRA planning.
