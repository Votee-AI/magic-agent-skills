...
**When NOT to use:** for parallel/bitext data → `linguistic-bitext`. For tokenizer-level audit → `linguistic-tokenize`. For per-dataset license/ethics → ALWAYS route through `linguistic-ethics` after the corpus catalog is identified.
...
7. **Confusable folding MUST happen before MinHash** (route to `linguistic-scripts`). Otherwise Cyrillic / Latin / Greek look-alike duplicates survive dedup.
...
Route to `linguistic-ethics` for per-dataset audit BEFORE adding to mix. Without this, you may be training on community-restricted data.
...
Route to `linguistic-scripts` for normalization policy. NFC + confusable-fold (dedup-key only) before MinHash.
...
**Recommended next step:** route to linguistic-tokenize for fertility audit
```
...
- **NEVER** add a dataset to the mix without routing through `linguistic-ethics` first.
...
**Hand-off:** linguistic-tokenize for fertility audit; linguistic-bitext if MT data also needed
```
