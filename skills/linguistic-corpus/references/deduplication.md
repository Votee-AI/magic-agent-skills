...
Loaded by `linguistic-corpus` Step 5.
...
Before MinHash:
1. Unicode normalize via `linguistic-scripts` (NFC).
2. Confusable fold via `linguistic-scripts` (TR39 — fold ONLY for the dedup key, not stored text).
