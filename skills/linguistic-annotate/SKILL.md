...
**When NOT to use:** purely synthetic-data generation → no human labels needed. Adapter / training itself → `linguistic-transfer`. Existing gold-standard reuse without modification → just use it.
...
**Hand-off:** linguistic-eval (gold for benchmark); linguistic-syntax/morph/etc. (gold for analysis)
```
