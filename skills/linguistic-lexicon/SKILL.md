...
2. **MWE inventories** drive RAG glossary injection + MT post-edit. "Kick the bucket" / "let the cat out of the bag" — without MWE-aware processing, MT silently mistranslates literally. (Cross-reference `linguistic-semantics/references/mwe_parseme.md`.)
...
7. **For low-resource lexicon construction**: bootstrap via cognate sets (cross-reference `linguistic-historical`) → community refinement → curator pass.
...
- `linguistic-semantics/references/wordnet_omw.md` — sense inventories at scale.
- `linguistic-semantics/references/mwe_parseme.md` — MWE handling.
- `linguistic-historical/references/canonical_sources.md` — cognate-based lexicon bootstrap.
